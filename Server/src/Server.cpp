...
{
	std::u16string filePath = u"Monitors.lmm";
...
void Server::saveMonitors()
{
	...
	writer.writeU16String(u"Lotova");
...
{
	std::u16string filePath = u"Scanners.lmm";
	BinaryReader reader(filePath);
	if (!reader.isOpen())
		return;
...
void Server::startUp()
{
	base = std::shared_ptr<Base>(BaseLoader::load(u"Base.lmm"));
	threats = std::make_shared<ThreatList>(u"Threats.lmm");
	threats->load();
